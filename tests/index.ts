...
///////
// IF ERROR OCCURS, WILL PRINTED IN CONSOLE
//////

// assert(50 > 70); // <-- will show `AssertionError [ERR_ASSERTION]: The expression evaluated to a falsy value:`

class Deno {
  static test(arg0: string, arg1: () => void) {
    console.log(arg0);
    arg1();
  }
}
...
Deno.test('serialize symbol', () => {
  ...
  assertEquals(serialize([symbol1, symbol2, Symbol('sym1'), [symbol2]]), '[$1,$2,$3,$4];Symbol("sym1");Symbol("sym2");Symbol("sym1");[$2]');
});
...
Deno.test('serialize build-in Map deep', () => {
  ...
  assertEquals(
    serialize(new Map<any, any>([['key1', map1] as const, [map2, 'val2'] as const])),
...
Deno.test('serialize build-in Map circular', () => {
  ...
  assertEquals(serialize(map1), 'Map($0=>$0)');

  const map2 = new Map<any, any>();
  map2.set(map2, 'val');
  map2.set('key', map2);

  assertEquals(serialize(map2), 'Map($0=>"val","key"=>$0)');
});
...
Deno.test('serialize regex', () => {
  ...
  assertEquals(serialize(/abc/gim), '/abc/gim');
});

Deno.test('serialize array', () => {
  assertEquals(serialize([]), '[]');

  assertEquals(serialize([[[{}, 2, '', false, []], [[]]], [1, 2], [1]]), '[$1,$2,$3];[$4,$5];[1,2];[1];[$6,2,"",false,$7];[$8];{};[];[]');

  assertEquals(serialize([{ name: 'wan2land' }, { name: 'wan3land' }]), '[$1,$2];{"name":"wan2land"};{"name":"wan3land"}');

  assertEquals(serialize({ users: [{ name: 'wan2land' }, { name: 'wan3land' }] }), '{"users":$1};[$2,$3];{"name":"wan2land"};{"name":"wan3land"}');
});

Deno.test('serialize object', () => {
  assertEquals(serialize({}), '{}');

  assertEquals(serialize({ foo: 'foo string', und: undefined }), '{"foo":"foo string","und":undefined}');
  assertEquals(serialize({ foo: { bar: 'bar string' } }), '{"foo":$1};{"bar":"bar string"}');
});
...
Deno.test('serialize object circular', () => {
  ...
  assertEquals(serialize(parent), '{"children":$1};[$2,$3];{"parent":$0,"next":$3,"siblings":$1};{"parent":$0,"next":$2,"siblings":$1}');
});
...
Deno.test('serialize class', () => {
  ...
  assertEquals(serialize(user), 'TestUser{"name":"wan2land","age":20,"publicSomething":2}');
});
...
Deno.test('serialize class with private', () => {
  ...
  assertEquals(serialize(user), 'TestUser{"name":"wan2land","age":20,"publicSomething":2,"privateSomething":1}');
});
